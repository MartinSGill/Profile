...
function script:Get-MyProTerminal {
    ...
    switch ($processName) {
        'code' {
            [MyProTerminals]::VsCode
        }
        'idea64' {
            [MyProTerminals]::JetBrainsJediTerm
        }
        'rider64' {
            [MyProTerminals]::JetBrainsJediTerm
        }
        'WindowsTerminal' {
            [MyProTerminals]::WindowsTerminal
        }
        { $PSItem -in 'explorer', 'conhost' } {
            [MyProTerminals]::WindowsConsole
        }
        'Console' {
            [MyProTerminals]::Console2Z
        }
        'ConEmuC64' {
            [MyProTerminals]::ConEmu
        }
        'FluentTerminal.SystemTray' {
            [MyProTerminals]::FluentTerminal
        }
        Default {
            [MyProTerminals]::Unknown
        }
    }
}
