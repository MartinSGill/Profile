    ...
    process {
        ...
        if ($fontFiles) {
            foreach ($item in $fontFiles) {
                Write-Verbose -Message "Processing font file {$item}"
                if (Test-Path (Join-Path -Path $objfontFolder -ChildPath $item.Name)) {
                    Write-Verbose -Message "Font {$($item.Name)} already exists in {$objfontFolder}"
                } else {
                    Write-Verbose -Message "Font {$($item.Name)} does not exists in {$objfontFolder}"
                    Write-Verbose -Message "Reading font {$($item.Name)} full name"
                    ...
                    Write-Verbose -Message "Font {$($item.Name)} full name is {$FontName}"
                    Write-Verbose -Message "Copying font file {$($item.Name)} to system Folder {$objfontFolder}"
                    ...
                    if (-not ($regTest)) {
                        ...
                        Write-Verbose -Message "Registering font {$($item.Name)} in registry with name {$FontName}"
                    }
                }
            }
        }
